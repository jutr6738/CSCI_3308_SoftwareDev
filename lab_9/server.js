...
app.post('/home/pick_color', function(req, res) {
    ...
    .then(info => {
    	res.render('pages/home',{
				...
				color_msg: color_msg // Return the color message of the color added to the table
			})
    })
    .catch(err => {
            console.log('error', err);
...
app.get('/team_stats', function (req, res) {
		var allgames = 'SELECT * FROM football_games;';
		var wincount = 'SELECT count(*) AS wincount FROM football_games WHERE home_score > visitor_score;';
		var losscount = 'SELECT count(*) AS losscount FROM football_games WHERE home_score < visitor_score;';
		...
		db.task('get-everything', task => {
			...
			.then(info => {
				...
				res.render('pages/team_stats', {
					my_title: 'Team Stats',
					data: info[0],
					wins: info[1][0].wincount,
					losses: info[2][0].losscount
				})
			})
			.catch(err => {
				console.log('error', err);
...
app.get('/player_info', function (req, res) {
	var playerquery = 'SELECT * FROM football_players;'; 
...
app.get('/player_info/post', function(req,res) {
	var getplayerid= req.query.player_choice;
	var allplayers = 'SELECT * FROM football_players;';
	var specificplayer= 'SELECT * FROM football_players WHERE  id=\' ' + getplayerid + ' \';';
	var gamesplayed= 'SELECT COUNT(*) AS gamesplayed FROM football_games INNER JOIN football_players ON football_players.id = ANY(football_games.players) WHERE football_players.id =\' ' + getplayerid + '\';';
	...
	.then(info => {
		res.render('pages/player_info', {
			...
			allplayers: info[0],
			selected: info[1][0],
			gamecount: info[2][0].gamesplayed
